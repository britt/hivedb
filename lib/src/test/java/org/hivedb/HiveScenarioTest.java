...
//		try {
//			final NodeResolver directory = new Directory(partitionDimension, new HiveBasicDataSource(hive.getHiveUri()));
...
//							Filter.grepFalseAgainstList(Arrays.asList(new Node[]{fromNode}), partitionDimension.getNodeGroup().getNodes())); // note, using Node instead of node uri since our tests use duplicate node uris
//					assertTrue(!directory.getNodeIdsOfPrimaryIndexKey(primaryIndexInstance.getPrimaryIndexKey()).contains(fromNode.getId()));
//				}
...
//	}
//	
//	private static Node getNode(Hive hive, NodeResolver directory, String dimensionName, Object primaryIndexKey) throws HiveException {
