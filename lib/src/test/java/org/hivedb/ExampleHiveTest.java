...
//		Product productA = (Product) sporkDaos.iterator().next().getJdbcTemplate().queryForObject(selectProductById, parameters, new ProductRowMapper());
//		//Make sure its a spork
//		Assert.assertEquals(spork.getName(), productA.getName());
//		
//		//Retrieve the spork by Name
//		sporkDaos = (Collection<SimpleJdbcDaoSupport>) hive.getJdbcDaoSupportCache(dimensionName).get(nameIndex, spork.getName(), AccessType.Read).iterator().next();
