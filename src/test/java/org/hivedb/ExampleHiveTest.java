...
public class ExampleHiveTest extends DerbyTestCase {
	...
	public void createAndUseTheHive() throws Exception {
		...
		JdbcDaoSupport sporkDao = hive.getJdbcDaoSupportCache(dimensionName).get(spork.getId(), AccessType.ReadWrite);
		...
		JdbcTemplate j = sporkDao.getJdbcTemplate();
		j.update(stmtFactory.newPreparedStatementCreator(parameters));
		...
		sporkDao = hive.getJdbcDaoSupportCache(dimensionName).get(spork.getId(), AccessType.ReadWrite);
		parameters = new Object[] {spork.getId()};
		Product productA = (Product) sporkDao.getJdbcTemplate().queryForObject(selectProductById, parameters, new ProductRowMapper());
		//Make sure its a spork
		Assert.assertEquals(spork.getName(), productA.getName());
		
		//Retrieve the spork by Name
		sporkDao = hive.getJdbcDaoSupportCache(dimensionName).get(nameIndex, spork.getName(), AccessType.ReadWrite);
		parameters = new Object[] {spork.getName()};
		Product productB = (Product) sporkDao.getJdbcTemplate().queryForObject(selectProductByName, parameters, new ProductRowMapper());
		//Make sure its a spork
