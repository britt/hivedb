...
import org.hivedb.hibernate.OpenSessionEventImpl;
...
public class HibernateTransactionHelper {
  ...
  public Collection<Object> queryInTransaction(QueryCallback callback, Session session) {
		...
		try {
			...
			log.debug("queryInTransaction: Error on data node " + OpenSessionEventImpl.getNode(), e);
			throw e;
		} finally {
			session.close();
		}
  ...
  public void updateInTransaction(SessionCallback callback, Session session) {
		...
		try {
			...
			log.debug("doInTransaction: Error on data node " + OpenSessionEventImpl.getNode(), e);
			if(tx != null)
				tx.rollback();
			throw e;
		} finally {
